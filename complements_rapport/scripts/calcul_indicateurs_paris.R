...
zoi <- read_sf("C:/Users/riley/Documents/GitHub/indicateurs_UHI/ventilation_mesoNH_maillage/zone_strat2_full.geojson")
# Fix the projection
# st_crs(zoi) <- 102110
st_crs(zoi) <- 2154
...
drops <- c("vgttn_s", "rt_srfc", "rt_lngr", "trttr_l", "hydr_sr", "hydr_ln", "floor","cmpc_mn_n", "cmpc_mn_w",
...
#            "icif", "icio", "id", "local", "pcif", "pcio", "pd", "psc", "typo_mj", "typ_scn")
# drops <- c("vegetation_surface", "route_surface", "route_longueur", "trottoir_longueur", "hydro_surface", "hydro_longueur",
#           "contig_std", "main_dir_std", "h_mean", "h_std", "b_area", "build_numb", "min_m_dist", "mean_m_dist", "mean_std_dist",
#            "b_std_compacity", "dist_to_center", "build_dens", "hydro_dens", "veget_dens", "road_dens", "ext_env_area", "ba", "bgh",
#            "icif", "icio", "id", "local", "pcif", "pcio", "pd", "psc", "typo_maj", "typo_second" )
...
# Convert zeros in ISF to NA values 
zoi$isf[which(zoi$isf == 0)] <- NA
...
names(permUSR)
# Drop unnecessary columns
drops <- c("code_ns", "dcomirs", "ins_ndv", "ins_mng", "ins_mn_c", "ins_mn_s", "ins_sr_", "TEB_T1", "TEB_T2", "TEB_T3",  
           "TEB_T4", "TEB_T5", "TEB_T6", "usr_area", "b_area", "m_height", "bsf", "hre", "r_area", "rsf", "imp_area", "isf",
           "osm_id")
...
perviousSurface <- data.frame("pk_usr" = permUSR_data$pk_usr, "v_area" = permUSR_data$v_area, "vsf" = permUSR_data$vsf, "p_area" = permUSR_data$p_area, "psf" = permUSR_data$psf)
# Join pervious surface data to ZOI
zoi <- full_join(zoi, perviousSurface, by = "pk_usr")

# Calculate the artificial surface fraction (addition of the building and impervious surface area)
permUSR_data$usr_area_num <- as.numeric(permUSR_data$usr_area)
permUSR_data$asf <- (permUSR_data$t_imp_area/permUSR_data$usr_area_num)
# Some fractions are greater than one which is incorrect, but the USR are less than 10m^2 in area, so their value is not significant
# We will assume that the areas are heavily artificialised and accord a default value of one
permUSR_data$asf[which(permUSR_data$asf > 1)] <- 1
# Change nan values to na 
permUSR_data$asf[which(permUSR_data$asf > NaN)] <- NA
# Export the afa in a separate data frame
artifical_data <- data.frame("pk_usr" = permUSR_data$pk_usr, "asf" = permUSR_data$asf)
# Join the artifical surface fraction to the ZOI
zoi <- full_join(zoi, artifical_data, by = "pk_usr")


# Export the ZOI
# st_write(zoi, "zoi.shp")
# st_write(zoi, "zoi_mesoNH_strat1.geojson")
# st_write(zoi, "zoi_mesoNH_strat2.geojson")
# st_write(zoi, "zoi_indicateurs_strat_2.geojson")
st_write(zoi, "strat_2_et_mapuce.geojson")
